...
import {
  saveQueryParamsOnLaunchAction,
  saveResultsAction,
} from "../../actions/mainAction.js";
...
class Main extends Component {
  ...
  searchAndSave = (type = "") => {
    let requestParam =
      this.state.searchKey.trim() === ""
        ? ""
        : `&needle=${this.state.searchKey.trim()}`;
    let getSelectedFilter = this.state.selectedFilterOption;
    ...
    if ((type === "search" && requestParam !== "") || type !== "search") {
      ...
      searchApi(requestParam, currentPage).then((response) => {
        ...
        let filterRender = isFilterExist
          ? this.state.filter
          : filter.sort(this.sortByDisplayOrder);
  ...
  render() {
    ...
    return (
      <>
        <Searchbar
          searchAndSave={this.searchAndSave}
        ...
        <div className="container-wrapper">
          <div className="container-fluid pt-4 mt-3 px-4">
            <div className="icon-grid-list">
              <FontAwesomeIcon
                icon={faThLarge}
                onClick={this.handleChangeViewGrid}
                className={`grid ${changeView ? "" : "active"}`}
              />
              <FontAwesomeIcon
                icon={faThList}
                onClick={this.handleChangeViewList}
                className={`list ${changeView ? "active" : ""}`}
              />
            </div>
            <div>
              <Filter
              ...
              {this.state.moreFilter ? (
                ...
                <div>
                  ...
                  />
                  {this.state.jsonData && this.state.jsonData.length === 0 ? (
                    ""
                  ) : (
                    <div className="load-more-bgcolor">
...
function mapDispatchToProps(dispatch) {
  return {
    saveQueryParamsOnLaunch: (params) =>
      dispatch(saveQueryParamsOnLaunchAction(params)),
